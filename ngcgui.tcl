...
# Standalone Usage (typ) create link:
#  $ ln -s somewhere/ngcgui.tcl destination_in_path/ngcgui
#
# Usage:
#   ngcgui --help | -?
...
#
#-----------------------------------------------------------------------
# ngcgui developed on git-master version 2.4.0-pre
...
# parameters must be monotonicly increasing with no omissions.  This
...
# All postional parameters must be entered -- an error occurs if an item
# entry is missing when a feature is made with "Create Feature"


# For _globals, additional information can be included on a special
...
# As in many languages, avoid use of _globals is recommended
...
# To allow subroutines to return values as named _globals, _globals with
# names that contain a colon (:) character are ignored in the creation of
# entry boxes
# example a subroutine called from a Subfile named o<line> returns
# results like: #<_line:theta>, $<_line:length> etc.
# This feature can be used to hide globals from entry boxes for any purpose or
# communication between routines

# Workflow:

#  1) The directory location for ngc gcode files used in emc2
#     is specified in the ini file by: [DISPLAY]PROGRAM_PREFIX

#  2) Candidate files for use with this utility should contain
#     a single subroutine as described in:
#          3.7 Calling Files of the EMC ________ manual
...
# Options:
#   "Retain values on next Subfile read"
...
# Notes:
#   0. ngcgui developed and tested under emc2-pre2.5
...
#
#      In the manual:
#        "Parameters from #n+1 to #30 have the same value as
...
#   3. EMC gcode supports labels for conditionals and subroutines
...
#   4. If one inadvertenly codes a _global name in the special association
...
#   8. entry boxes for _global and postional parameters include a key bindings
#      keys x,y,z,a,b,c,u,v,w,d.  When embedded in axis, typing these keys
...
#
#      (If there is a global ::entrykeybinding proc, it will
...
#  15. Some helper subroutine files that are included in the
#      [DISPLAY]PROGRAM_PREFIX directory may not be
#      suitable for use as a subfile.  To indicate this to a
#      user, include a comment line:
#          (not_a_subfile)
...
#
# 17. xembed support removed, internal embedding works better
#
# 18. If --vwidth 0 is used and a parameter has no comment, the variable
#     name is placed in the comment field
#
# 19. The proc embed_in_axis_tab will embed directly in an axis tab using
#     [DISPLAY]USER_COMMAND_FILE (or ~/.axisrc)
#     example:
...
#     output gcode file
#
# 21. When embedding in axis directly,  multiple tabpagess can be specified.  Each
#     can be used independently to add multiple features from the initial or
#     newly selected subfiles.  If multiple tabpagess have created features, the
#     Finalize action will offer to finalize all tabpagess in left-to-right order.
#     Beware of this ordering. You may rearrange page order before finalizing.
#
...
#puts "parse $hdl $filename"
  new_image $hdl $filename

  if {$ay($hdl,retainvalues)} {
    # _globals:             retain all
    # postional parameters: retain some
...
# wC create frame
      ...
      set b [button $w.[qid] -text "Creat Feature" -font $::ngc(any,font) \
            -width 14 -padx 1\
...
# wP positional parameters
      ...
      if ![parse $hdl ::ngc $::ngc($hdl,fname,subfile)] {
